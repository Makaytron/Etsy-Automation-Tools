...
- Listing Analyzer `v1.0.0` yalnız onaylanmış başlık, açıklama, etiket ve materyal alanlarını editöre aktarır; her listing için Etsy Publish öncesinde kullanıcı onayı bekler. Deaktif etmede script yalnız Etsy seçenek menüsünü açıp ilgili öğeye odaklanır; Deactivate ve final onayı kullanıcı tıklar. Delete işlemini otomatikleştirmez. AI değişimi ağsız JSON/prompt dışa aktarımı ve doğrulanmış teklif içe aktarımıdır; script Etsy veya AI API anahtarı istemez.
