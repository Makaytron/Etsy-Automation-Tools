...
- Listing Analyzer `v1.2.2` yalnız onaylanmış başlık, açıklama, etiket ve materyal alanlarını editöre aktarır; her listing için Etsy Publish öncesinde kullanıcı onayı bekler. Deaktivasyonda ayrı listing onayından sonra güncel sağlık, bağlam, güvenlik kapıları, anomali durumu, öneri temeli ve aktifliği hem navigasyondan önce hem final tıklama kilidi altında yeniden hesaplar. Yalnız tek, görünür ve etkin tam eşleşen **Deactivate** menü öğesi ile doğru başlıklı modalın özel final **Deactivate** düğmesini tıklar; stale ve eski uyumluluk incelemeleri bu yolu tetikleyemez. **Delete** semantiği, yanlış/çift modal, disabled veya değişmiş kontrol sıfır tıklamayla reddedilir. Gönderim niyeti final tıklamadan önce kalıcılaştırılır; belirsiz sonuç otomatik tekrarlanmaz ve kuyruk yalnız `Active → Inactive` doğrulamasıyla ilerler. AI değişimi ağsız JSON/prompt dışa aktarımı ve dışa aktarılan tam listing payloadı, kayıtlı öneri ve doğrulanmış deaktivasyon durumuna bağlı, doğrulanmış atomik öneri içe aktarımıdır; script Etsy veya AI API anahtarı istemez.
