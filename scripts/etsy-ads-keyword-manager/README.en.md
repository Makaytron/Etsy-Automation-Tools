...
When installed from the canonical GitHub file, the script checks version metadata at most once per 24 hours after the panel loads. Click the `v1.0.3` badge or use **Makaytron · Check script version** in the Tampermonkey menu for a manual check. The check reads only the remote `.user.js` `@version` value and never executes remote code. A network failure does not block the keyword tool.
