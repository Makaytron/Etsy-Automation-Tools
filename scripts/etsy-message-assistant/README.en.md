...
- In an individual conversation, Turkish translations appear directly below up to the latest 40 customer and seller messages without changing their source bubbles. Sources detected as Turkish get no redundant note.
- AI replies use real paragraph breaks. The normal **Müşteriye Gönder (Send to Customer)** action revalidates the conversation, unchanged draft, single composer, ownership locks, and Etsy Send control, dispatches once, and consumes the draft after matching outgoing-bubble verification.
