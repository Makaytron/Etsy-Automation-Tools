...
| `/messages/<conversation-id>` with exactly one trusted composer | **Messages** | Translations in the selected display language below customer and seller messages, Turkish drafts, multi-paragraph AI replies, templates, copy, and a verified **Send to Customer** action. |
...
The conversation list reads only safe links currently visible in Etsy's page and displays at most 50 rows. The **Display language** choice is persisted; list translations stay inside the panel and the source preview remains under **Show original message**. In one open conversation, a translation of up to the latest 40 customer and seller messages into the selected display language appears as a separate note immediately below each unchanged source bubble. A source detected as already matching the target gets no redundant note. If DeepL does not support the selected target, the inline note says so explicitly and uses Google only when **Free fallback** is enabled.
...
> **Privacy warning:** The panel is closed by default on message pages, but Google Translate is the default provider and **Automatic Translation Preview** is enabled by default. With that setting on, visiting an individual conversation may send up to the latest 40 customer and seller messages to the selected provider for translation into the selected display language even while the panel is closed, using no more than three concurrent requests and reusing identical/cached results. Disable automatic preview in **Makaytron Settings** from the Tampermonkey menu before opening a conversation if you do not want that transfer. Opening the panel on the conversation list may translate up to 50 visible previews; changing **Display language** may rebuild both open-conversation notes and list previews for the new target. Selecting **Translate previews / Retry** also starts list requests. If DeepL fails while **Free fallback** is enabled, translation may fall back to Google. Other, non-review delivered-order templates may send the latest message to the selected translation provider to determine the target language even when automatic preview is off. The dedicated review-request template skips that language-detection transfer; choosing AI drafting can still send the context described below.
...
2. When automatic translation is on, read the translation note—labelled with the selected **Display language**—immediately below both customer and seller messages. **Customer message / Your message** labels, distinct colors, and opposing alignment distinguish the sides; source bubbles remain unchanged.
3. The panel is optional. Use **Makaytron hızlı cevap (Makaytron quick reply)** beside Etsy's composer, or open the panel from **Assistant · Open** for detailed options. **Müşterinin Mesajı (Customer Message)** and the manual selected-display-language action provide a fallback view of the latest message.
4. Write your Turkish response in Etsy's composer or the panel draft field; open the panel to choose **Hazır mesaj ekle… (Insert Saved Template)**.
...
6. In the quick bar, an empty Etsy composer enables **Suggest an AI Reply**; typed text enables **Polish with AI** and **Translate Only**. The result replaces the composer only if its exact text and conversation stayed unchanged while the request ran. Review the result; AI output separates the greeting, short body paragraphs, and optional closing with real line breaks.
7. Select the shared **Müşteriye Gönder (Send to Customer)** action in either the panel or quick bar. The script revalidates the current conversation, exact Etsy composer text, single composer, Etsy's one enabled Send control, and competing send owners; it then dispatches once and waits for a matching outgoing bubble.

The panel and quick bar get their labels and behavior from the same shared message-action definitions, so a behavior change applies to both surfaces. After successful verification, the draft is consumed and cannot be dispatched a second time. A conversation, text, or ownership change fails safely. If Etsy's Send control does not become enabled after insertion, the draft remains in the composer but is not sent. If the post-click outcome is inconclusive, do not retry until you inspect the latest outgoing bubble in that Etsy conversation.
