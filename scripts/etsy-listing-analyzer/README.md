...
- Listing bağlantısından ID’yi çıkarır ve karttaki başlık, SKU, stok, fiyat, yenileme/bitiş metni ile doğrulanmış Etsy istatistik bölümlerini okur: ziyaret/favori yalnız **Son 30 gün**, satış/gelir/yenileme yalnız **Tüm zamanlar** kapsamından alınır. Gerçek aktif/taslak/süresi dolmuş/tükenmiş/pasif durumunu Etsy’nin durum filtresinden ayrı doğrular.
- **Tüm sayfaları tara** düğmesi veya `Ctrl + Alt + A` ile Etsy listing sayfalarını 1’den son sayfaya kadar sırayla toplar, ardından yeniden 1. sayfaya döner ve analiz kartlarını yalnız bu dönüş tamamlanınca gösterir. Her sayfada numaralandırmayı, görünür ham kart köklerini, kanonik düzenleme bağlantılarını, ayrıştırılan listingleri, benzersiz ID'leri ve metrik kapsam sözleşmesini doğrular; bu kümelerin sayıları birebir eşleşmeden ve aynı eksiksiz içeriği art arda üç kez görmeden kaydetmez. Hatalı/yinelenen ID'li kart, eksik/yinelenmiş/yanlış bölüme yerleşmiş metrik, yinelenen/örtüşen sayfa veya geçici DOM karışımı analiz kilidini açmaz. Geçici kart/yükleme ve sayfa geçişi hatalarını artan bekleme ile üç kez dener; sonuç alamazsa sayfa, aşama, deneme ve teknik sayaçları içeren güvenli bir ayrıntılı rapor üretir. Aynı düğme taramayı durdurur ve kaldığı yerden devam ettirir. Revizyonlu manifest ve içerik parmak izi, eski bir sekmenin daha yeni toplanmış sayfaların üzerine yazmasını engeller.
...
- Arama; kayıt kapsamı, yaşam döngüsü, sorun/fırsat, öneri, son 30 günlük performans, değişim, stok ve kanıt yeterliliği filtreleri; her seçenekte bağlama göre sonuç adedi, altı hazır preset, en fazla sekiz özel preset, öncelik/30 günlük erişim-ilgi/ziyaret/tüm-zaman satış-gelir/kanıt yeterliliği sıralaması ve sonuç sayacı sunar. Büyük mağazalarda kartlar 40’lık gruplarla yüklenir.
...
- Health Engine değerlendirme bağlamında yaşam döngüsünü, performans hipotezini, sezgisel **Kanıt yeterliliği** göstergesini, karar kanıtlarını ve sonraki inceleme zamanını birlikte ele alır.
...
- Yeterli ve karşılaştırılabilir kayıt bulunduğunda yalnız güncel, tamamlanmış taramadaki listinglerden mağaza içi karşılaştırma grubu (cohort) kurar; hedef listingi, eski/anomalili kayıtları ve tarama dışı kayıtları emsal yapmaz. Kohort en az sekiz emsalle kullanılabilir olur ve etkisi 30 emsale kadar kademeli artar; kesin yüzdelik tanısı ise dayandığı metriğin kendi emsal grubunda tam güce ulaşmasını gerektirir. Bilinen ürün türü, mevsimsellik, fiyat bandı ve para birimi emsal kapsamını daraltır.
- Geçmiş ayrıntısında ziyaret, favori, satış, gelir ve yenileme için erişilebilir inline SVG değişim grafikleri gösterir; eksik değerleri sıfıra dönüştürmez. Yaklaşık ve eski-format noktaları işaretler, sınırlı kanıtlı uçlarda yön rengi üretmez, bayat gözlemleri boşluk olarak bırakır ve tümü sıfır olan seriyi sıfır taban çizgisinde tutar. Gelir grafiği kanonik para birimi kimliklerini kullanır; geçmişte birden fazla birim varsa yalnız en güncel birimin serisini göstererek farklı birimleri aynı sayısal ölçekte karşılaştırmaz.
- İyileştirme önerilerini, yayın anına en fazla bir gün uzaklıktaki başlangıç snapshotını ve doğrulanmış yayınlama sonucunu kaydeder; planlama, yayınlama, gözlem, değerlendirme tarihi ve sonucu ayrı olaylarla gösteren deney zaman çizelgesi oluşturur. Deney sinyallerini %95 exact conditional Poisson oran-oranı aralığıyla sınırlar; sıfırdan pozitife geçişte sahte yüzde üretmez. Satış için gerçek, eşlenmiş 30 günlük kümülatif-satış pencerelerini kullanır ve 30. günden sonra geçerli snapshot için yedi gün bekler.
- Seçili listingler için gerçek listing ID’lerini dışarı vermeyen `L001` benzeri geçici referanslarla AI istek JSON’u oluşturur. Her referans tam düzenlenebilir içeriğe, analiz temeline ve dışa aktarılan payload parmak izine bağlanır.
- Doğrulanmış AI yanıt JSON’unu başlık, açıklama, etiket ve materyal önerisine dönüştürür; yalnız değişmesi istenen alanlar için **Önce / AI önerisi / Doğrulanan sonuç** karşılaştırması gösterir. Dışa aktarımdan sonra referanslanan listing, kayıtlı öneri veya doğrulanmış deaktivasyon durumu değişmişse içe aktarımın tamamı atomik olarak reddedilir.
...
Health Engine yalnız Etsy Shop Manager listing kartında görünür olan başlık, SKU, stok, fiyat, durum ve kapsamı DOM başlıklarıyla doğrulanmış metriklerle çalışır: ziyaret/favori **Son 30 gün**, satış/gelir/yenileme **Tüm zamanlar** bölümünden gelmelidir. Eksik, yinelenmiş veya yanlış bölümdeki değerler fail-closed reddedilir; kapsamı doğrulanamayan eski snapshotlar karar çıpası olamaz. Etsy API'sine, mağaza dışı benchmark verisine veya harici bir toplama servisine bağlanmaz. Etsy kaynak yenileme zamanı görünmediği için güncellik tarama zamanına göredir. Snapshotlar yalnız kullanıcı **Bu sayfayı tara**, **Tüm sayfaları tara** veya tam taramayı başlatan **Analizi başlat** işlemini kullandığında yerel Tampermonkey alanında oluşur.

Ziyaret ve favori değerleri kayan son 30 günlük pencere; satış, gelir ve yenileme değerleri ise tüm zamanlar toplamıdır. Bu nedenle motor, iki tarama arasındaki bütün metrikleri aynı tür fark gibi yorumlamaz. Trend ve deaktivasyon çıpaları yalnız listingin mevcut kesintisiz **aktif durum dönemi** içinden seçilir; aradaki pasif dönem yeni bir dönem başlatır. Exact satış ve yenileme farkları kendi exact çıpalarını seçer; gelir farkı için iki gözlemin para birimi aynı olmalıdır. Exact kayan-pencere trendi için ziyaret sayaçları exact olmalı ve 30–31 gün uzaktaki örtüşmeyen bir çıpa bulunmalıdır; daha yakın, daha eski veya yaklaşık sayaçlar yalnız açıklayıcı kalır. Kısa aralıklı değişimler erken sinyal sayılır, daha uzun ve yeterli örneklem içeren pencereler daha hazır kanıt sağlar.

Karttaki **30 günlük erişim/ilgi** puanı son 30 günlük ziyaret ile örneklem boyutuna göre yumuşatılmış favori oranını ölçer; favori bileşeninin ağırlığı yeterli ziyaret oluşana kadar kademeli artar ve aynı anlamı ilk ve sonraki taramalarda korur. Tüm-zaman satış/gelir kanıtı listingi riskli değişikliklerden koruyabilir, yenilemeler ise verimsizlik önceliği oluşturabilir; bu tarihsel sayaçlar güncel erişim/ilgi puanını yapay olarak yükseltmez. **Kanıt yeterliliği** veri kapsamını ve karar vermeye hazırlığı özetleyen sezgisel bir göstergedir; önerinin doğru olma olasılığı değildir. İlk taramada düşük olması, eksiksiz okunan sıfır değerlerin “veri yetersiz” sayılması değildir; büyüme/düşüş için ardışık 30 günlük, deaktivasyon incelemesi için en az 58 günlük kesintisiz aktif geçmiş ve diğer güvenlik kanıtları yine zorunludur.
...
- **Kanıt yeterliliği:** veri kalitesi, geçmiş derinliği, trafik örneklemi, karşılaştırma grubu, güncellik ve tutarlılıktan oluşan sezgisel hazırlık puanıdır; istatistiksel olasılık değildir. Hedef hariç sekiz karşılaştırılabilir emsal kohortu kullanılabilir kılar, kohort etkisi 30 emsale kadar kesintisiz artar. Eksik veri veya kısa geçmiş kesin bir karar için yeterli sayılmaz.
...
Listing ayrıntısındaki **Listing bağlamı** kartından mevsimselliği `Mevsimsel`, `Mevsimsel değil` veya `Bilinmiyor`; ürün türünü `Dijital`, `Fiziksel` veya `Bilinmiyor` olarak kaydedebilirsiniz. Bilinen değerler karşılaştırma grubunu aynı bağlama böler. Deaktivasyon incelemesinin mevsimsellik kapısı yalnız `Mevsimsel değil` açıkça seçildiğinde geçer.

Karşılaştırma grubu yalnız aynı mağazada, bu tarayıcıda birikmiş ve anlamlı biçimde karşılaştırılabilen yerel kayıtlardan oluşturulabilir. Hedef listing kendi emsalleri arasında yer almaz; yeni değiştirilmiş, deneydeki, pasif, stoksuz veya eksik verili listingler dışlanır. En az sekiz emsal yoksa kohort güvenilir sayılmaz; göreli ağırlık sekizden 30 emsale kadar kademeli artar. Bu kıyas Etsy geneli pazar verisi değildir.

Kaydedilmiş bir iyileştirme, yayın anına en fazla bir gün uzaklıktaki başlangıç snapshotı ve değiştirilen alanlarla birlikte gözlemsel deney bağlamında karşılaştırılabilir. Başlık/etiket deneyleri ziyaret değişimini, materyal deneyleri favori/ziyaret oranını, açıklama deneyleri ise satış/ziyaret oranını ölçer. **Değişiklik sonrası artış/azalış sinyali** yalnız %95 exact conditional Poisson oran-oranı güven aralığı 1 değerini dışladığında verilir. Başlangıç sıfırken pozitif sonuç sahte bir göreli yüzdeye çevrilmez; mutlak yeni sinyal olarak gösterilir. Satış karşılaştırması normalize edilmiş kesirler yerine başlangıç ve değerlendirme çevresindeki eşlenmiş gerçek 30 günlük pencerelerin tamsayı satış farklarını kullanır. Motor 30. günde uygun snapshot yoksa yedi günlük grace süresince `İzleniyor` durumunda bekler; süre sonunda hâlâ geçerli veri yoksa `Belirsiz` kalır. Son 30 gün metrikleri kayan pencere olduğundan sonuç nedensellik iddiası değildir ve erken kontroller yalnız ön sinyaldir. Aynı listingde üst üste yapılan değişiklikler yorumu zayıflatacağından kontrollü, tek hipotezli ilerleme önerilir.

`Kapatmayı incele` tek başına otomatik kapatma emri değildir. Eksik veri, düşük kanıt yeterliliği, kesintisiz aktif geçmişin yetersizliği, bilinmeyen/mevsimsel bağlam, yeni değişiklik veya devam eden deney güvenlik kapısıdır. Adaylık oluşsa bile kullanıcı listing bazında deaktivasyonu ayrıca onaylar. Güncel sağlık, bağlam, güvenlik kapıları, anomali durumu, öneri temeli ve aktiflik Etsy açılmadan önce ve final tıklama kilidi altında yeniden hesaplanır. Ardından script tam DOM sözleşmesini yeniden doğrular, yalnız **Deactivate** ve final **Deactivate** kontrollerine tıklar ve sonucu görünür `Active → Inactive` geçişiyle doğrular. Doğrulanan durum değişikliği eski tamamlanmış koleksiyonu geçersiz kılar; analiz, AI dışa aktarımı ve yeni kuyruk için taze tam tarama gerekir. Eski manuel kuyruk kayıtları yalnız doğrulama içindir ve otomatik tıklamayı tetikleyemez.
...
- Tüm-sayfa veri toplamada ayrı sekme kilidi, sayfa imzası, tekrar döngüsü sınırı ve en fazla 250 sayfalık güvenlik tavanı kullanır. Aktif işlem kuyruğu ile tüm-sayfa taraması aynı anda çalışamaz; eski sürümden kalmış çakışmalı durumda yalnız kuyruğu durduran güvenli kurtarma yolu açıktır. Durdurulan adımın Etsy sonucu belirsizse çakışan tarama da atomik olarak bloklanır ve yeniden tam tarama gerekir. Tarama yalnız görünür listing verisini okur; listinglere yazmaz.
...
- Native Web Locks depolama güncellemelerini sekmeler arasında sıraya alır; kayıt birleştirme ve yazma doğrulaması eşzamanlı snapshot/öneri değişikliklerini korur. Revizyonlu collection compare-and-swap yazımı, kanonik manifest parmak izi ve gerçek sayfa gözlem zamanı; eski durdur/devam sekmelerinin veya okuma sonrası durum değişikliklerinin güncel koleksiyon sayılmasını engeller. Doğrulanmış deaktivasyon aynı mağazadaki etkilenen aktif ve inaktif kapsamları geçersizleştirir. AI içe aktarma ve kuyruk oluşturma, işlem anında aynı güncel ve eksiksiz tam-tarama kimliğiyle dışa aktarılan tam payloadı yeniden doğrular.
