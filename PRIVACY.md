...
- Google Translate varsayılan çeviri sağlayıcısıdır. Varsayılan açık otomatik Türkçe önizleme, mesaj sayfası açıldığında son müşteri mesajını `translate.googleapis.com` adresine gönderebilir; manuel ücretsiz çeviri de seçilen metni aynı alıcıya gönderir.
...
- Etsy Sale Manager, Message Assistant, Ads Keyword Manager ve Keyword & Market Analyzer uygulama içi güncelleme denetimleri en fazla 24 saatte bir kendi canonical public `raw.githubusercontent.com` userscript kaynaklarını okuyabilir. Başka dağıtım kaynağından kurulumda özel GitHub akışı zorlanmaz.
