...
| [Makaytron Etsy Message Assistant](./scripts/etsy-message-assistant/README.md) | 1.2.5 | Çevrilmiş gelen kutusu önizlemesi, cevap taslağı, yorum talebi uygunluk kontrolü ve açık opt-in ile alıcıları tek tek işleyen premium sade Otomasyon/Otopilot paneli sunar. |
...
| [Makaytron Etsy Listing Analyzer](./scripts/etsy-listing-analyzer/README.md) | 1.1.0 | Tüm sayfaları tek komutla sırayla toplar, ilk sayfaya dönünce analizi açar; yeniden deneme, hata raporu, presetler, grafikler, AI karşılaştırması ve Health Engine sunar. |
...
- Listing Analyzer `v1.1.0` AI ağına bağlanmaz: anonimleştirilebilir istek JSON'u/prompt'u dışa aktarır ve doğrulanmış teklif JSON'u içe alır. Her listing Etsy Publish veya deaktivasyon öncesinde kullanıcı onayı bekler. Listing bazındaki deaktivasyon onayından sonra script yalnız Etsy'nin tam eşleşen **Deactivate** menü öğesini ve son **Deactivate** düğmesini seçer; yalnız görünür `Active → Inactive` geçişinden sonra ilerler. **Delete** hiçbir zaman seçilmez; doğrulanamayan gönderim otomatik tekrarlanmaz.
...
- Message Assistant varsayılan olarak kapalı kalır. Otomatik Türkçe önizleme açıktır; ancak son müşteri mesajını yalnız kullanıcı paneli açtığında veya otomatik açılmayı ayrıca etkinleştirdiğinde Google Translate'e gönderebilir. Bu aktarımı istemiyorsanız paneli kapalı tutun ya da açmadan önce otomatik önizlemeyi kapatın.
