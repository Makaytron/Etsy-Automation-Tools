...
`config/production-packages.json` is the canonical machine-readable production package inventory. Package id/slug, public userscript name, exact script path, and Greasy Fork id/slug must be changed there first and kept aligned with the actual `scripts/**/*.user.js` inventory, `tools/Mkui-Production-Registry.mjs`, `DISTRIBUTION.md`, release packaging, and hosted validation. Do not maintain a second independent Greasy Fork or production-script mapping in tooling.

Adding a new production userscript is a distribution-schema change, not just a new file. Before it can ship, add it to `config/production-packages.json` and extend all affected registries/gates deliberately: MKUI production registry/drift coverage where applicable, root/package documentation, hosted-channel data, and a dedicated product CI. The new script must then follow the same update URL, SemVer, documentation, identity, privacy, packaging, and release rules from its first reviewed release.

Active `SECURITY.md`, `SECURITY.en.md`, `PRIVACY.md`, and `PRIVACY.en.md` describe current production behavior and must stay version-agnostic for product behavior claims. Historical version-specific behavior belongs in release/advisory or migration records, not in the active policy prose. The Turkish and English privacy-policy update dates must stay synchronized.
