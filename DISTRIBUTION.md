...
2. Let the changed package's read-only product CI complete, then run `powershell -NoProfile -ExecutionPolicy Bypass -File tools/Test-Distribution.ps1 -Online` for local version, syntax, updater behavior, secret-pattern, and URL-reachability checks; review `git diff --check` as well. Every product CI exercises the shared `tools/New-Standalone-ReleaseAssets.ps1` builder and creates the same byte-identical `.user.js` plus `SHA256SUMS.txt` candidate. The candidate is uploaded as a workflow artifact only for manual or standalone-tag runs; CI never creates a tag or publishes a release. The historical `tools/New-Message-Assistant-ReleaseAssets.ps1` command remains a backward-compatible wrapper around the shared builder.
3. For an actual suite release, update `VERSION` and `docs/releases/v<VERSION>.md`, then let Suite Release CI run `tools/Test-Suite-Release-Assets.ps1`. The shared suite builder `tools/New-Suite-ReleaseAssets.ps1` creates exactly seven files from the reviewed Git tree: `Etsy-Automation-Tools-v<VERSION>.zip`, the five registry-listed byte-identical userscripts, and `SHA256SUMS.txt` covering the ZIP and all five scripts. Repeated builds from the same commit must be byte-identical. Non-tag builds are packaging regression checks only; the suite candidate artifact is uploaded only on the signed suite tag run.
4. Create a signed commit and a scope-matching signed tag from the reviewed commit: `vX.Y.Z` for a suite release or `<package-slug>-vX.Y.Z` for one standalone package. The corresponding tag CI must pass before publication; it verifies the exact tag/version/HEAD relationship and requires GitHub to report both the annotated tag signature and the release commit signature as verified. Local tag validation falls back to `git verify-tag` and `git verify-commit`.
5. After pushing canonical `main`, run `powershell -NoProfile -ExecutionPolicy Bypass -File tools/Test-Distribution.ps1 -Online -RemoteParity` to prove that every public Raw file is byte-equivalent to its local source.
6. Publish the complete GitHub Release once using only the verified release-candidate asset set from the matching reviewed tag. The release-only webhook requests an immediate Greasy Fork refresh from the configured exact Raw `main` paths; Greasy Fork's own automatic synchronization may also poll those paths.
...
- GitHub Actions has read-only repository permission and runs local tests, fixture smoke validation, distribution checks, deterministic release-candidate packaging, and signed-tag verification. It never synchronizes a host, creates a tag, or publishes a release.
