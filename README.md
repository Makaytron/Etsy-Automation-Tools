...
| [Makaytron Etsy Listing Analyzer](./scripts/etsy-listing-analyzer/README.en.md) | 1.1.0 | Collects every page in order, returns to page 1, and adds retries, error reports, presets, charts, AI comparison, and Health Engine workflows. |
...
- Listing Analyzer `v1.1.0` has no AI network integration: it exports an anonymizable request JSON/prompt and imports validated proposal JSON. Every listing waits for user confirmation before Etsy Publish or deactivation. After a per-listing deactivation confirmation, the script selects only Etsy's exact **Deactivate** menu item and exact final **Deactivate** button, then advances only after a visible `Active → Inactive` transition. **Delete** is never selected; an unverified submission is never retried automatically.
