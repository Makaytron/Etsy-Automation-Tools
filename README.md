...
| [Makaytron Etsy Message Assistant](./scripts/etsy-message-assistant/README.en.md) | 1.1.1 | Provides translation, reply drafts, review-request eligibility controls, guided send-and-next, templates, and user-selected AI provider profiles. |
...
- Read every generated message before sending it. Automatic sending is off by default; review requests ignore that global setting and require a separate **Send and Go to Next** click for every recipient.
...
- Message Assistant stays closed by default. Automatic Turkish preview is enabled, but it can send the latest customer message to Google Translate only after the user opens the panel or explicitly opts in to automatic opening. Keep the panel closed or disable automatic preview before opening it if you do not want that transfer.
