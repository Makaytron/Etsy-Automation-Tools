...
- Listing Analyzer `v1.2.1` fills only approved title, description, tag, and material fields, then waits for user confirmation before Etsy Publish for every listing. After a separate per-listing deactivation confirmation, it recomputes current health, context, safeguards, anomaly state, proposal basis, and active status before navigation and under the final-click lock. It clicks only one visible, enabled, exact **Deactivate** menu item and the dedicated final **Deactivate** button in the correctly titled modal; stale and legacy review items cannot trigger that path. Deletion semantics, wrong/duplicate dialogs, and disabled or replaced controls fail with zero clicks. Submission intent is persisted before the final click, an uncertain result is never retried automatically, and the queue advances only after an `Active → Inactive` verification. AI exchange is network-free request JSON/prompt export plus validated, atomic proposal import bound to the exact exported listing payload, saved proposal, and verified-deactivation state; the script requests neither Etsy nor AI API credentials.
- Listing Analyzer retries temporary page reads/navigation at most three times; storage, schema, ownership, and Etsy-write failures are not retried. Technical error reports exclude session data and page content. Listing Analyzer and Keyword & Market Analyzer update checks verify GitHub's `main` commit identity, read only the immutable commit Raw path, and open that same verified URL in Tampermonkey's confirmation screen.
