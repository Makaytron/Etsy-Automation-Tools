...
- Listing Analyzer `v1.0.0` fills only approved title, description, tag, and material fields, then waits for user confirmation before Etsy Publish for every listing. For deactivation, it only opens Etsy's options and focuses the relevant item; the user clicks Deactivate and the final confirmation. Delete is never automated. AI exchange is network-free request JSON/prompt export plus validated proposal import; the script requests neither Etsy nor AI API credentials.
