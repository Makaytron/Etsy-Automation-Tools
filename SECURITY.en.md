...
- Listing Analyzer `v1.1.0` fills only approved title, description, tag, and material fields, then waits for user confirmation before Etsy Publish for every listing. After a separate per-listing deactivation confirmation, it clicks only one visible, enabled, exact **Deactivate** menu item and the dedicated final **Deactivate** button in the correctly titled modal. Deletion semantics, wrong/duplicate dialogs, and disabled or replaced controls fail with zero clicks. Submission intent is persisted before the final click, an uncertain result is never retried automatically, and the queue advances only after an `Active → Inactive` verification. AI exchange is network-free request JSON/prompt export plus validated proposal import; the script requests neither Etsy nor AI API credentials.
