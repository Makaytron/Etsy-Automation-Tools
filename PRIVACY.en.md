...
- Listing Analyzer `v1.0.2` makes no network request to an AI provider and stores no AI API key. The user can export or copy an anonymizable request JSON/prompt, then import proposal JSON from an external tool after validation. The user controls what is sent to that external tool.
...
- Etsy Sale Manager, Message Assistant, Ads Keyword Manager, and Keyword & Market Analyzer in-app update checks may read their own canonical public userscript sources on `raw.githubusercontent.com` at most once every 24 hours. A non-GitHub installation source does not have the private GitHub flow forced over its distributor.
