...
- Listing Analyzer `v1.2.1` makes no network request to an AI provider and stores no AI API key. The user can export or copy an anonymizable request JSON/prompt, then import proposal JSON from an external tool after validation. The user controls what is sent to that external tool.
...
After the user separately confirms deactivation for one listing, Listing Analyzer clicks only the exact **Deactivate** menu item and final **Deactivate** button in the open Etsy editor. It does not use **Delete**. The operation identity and `Active → Inactive` verification state remain in the local action queue; an uncertain result is not submitted again automatically.
