...
- Listing Analyzer `v1.1.0` makes no network request to an AI provider and stores no AI API key. The user can export or copy an anonymizable request JSON/prompt, then import proposal JSON from an external tool after validation. The user controls what is sent to that external tool.
