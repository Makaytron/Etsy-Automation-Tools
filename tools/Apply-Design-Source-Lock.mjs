...
function insertBefore(text, anchor, marker, section, path) {
  ...
  return text.replace(anchor, `${section.trim()}\n\n${anchor}`);
}

function insertAfter(text, anchor, marker, section, path) {
  if (text.includes(marker)) return text;
  const occurrences = count(text, anchor);
  if (occurrences !== 1) {
    throw new Error(`${path}: expected one anchor ${JSON.stringify(anchor)}, found ${occurrences}.`);
