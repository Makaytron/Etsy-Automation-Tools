...
for (const hook of requiredHooks) assert(count(source, hook) > 0, `Missing pre-migration behavior hook: ${hook}`);
...
styles = replaceOnce(styles, '.maw-modal-body{padding:18px;overflow:auto;background:#f7f7f7}', '.maw-modal-body{padding:18px;overflow:auto;background:#f7f7f7}', 'modal body invariant');

let output = head + styles + tail;
